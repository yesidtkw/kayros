...
// Consulta productos
$sql = "SELECT id, nombre, marca, categoria, precio,condicion FROM productos";
...
while ($p = $resultado->fetch_assoc()) {
    $categoria = strtolower(str_replace(['á','é','í','ó','ú'], ['a','e','i','o','u'], $p['categoria']));

    // Establecer color según categoría
      if (in_array($categoria, ['miniportatil','miniportatil', 'mini portatil', 'mini-portatil'])) {
          $r = 173; $g = 216; $b = 230; // azul claro
      } elseif (in_array($categoria, ['portatil', 'portátiles', 'portátiles'])) {
          $r = 212; $g = 252; $b = 212;
...
$pdf->Output("I", "rotulos_con_logo.pdf");

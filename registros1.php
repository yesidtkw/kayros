...
<nav style="background:#0073e6;color:white;padding:15px 20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;">
  ...
  <script>
    ...
    [filtroCliente, filtroProducto, filtroFecha].forEach(input => {
      input.addEventListener("input", cargarDatos);

              if (tabla.innerHTML === "") {
                tabla.innerHTML = '<tr><td colspan="12">No se encontraron resultados</td></tr>';
              }
    ...
    // ⬇️ Aquí pegas la función listar(id)
    function listar(id) {

      fetch('pdf_registro.php?id=' + id, { method: 'HEAD' })
        .then(response => {
          if (response.ok) {
            window.open('pdf_registro.php?id=' + id, '_blank');
          } else {
            alert('⚠️ No se encontró el registro o hubo un error al generar el PDF.');
          }
        })
        .catch(error => {
          console.error('Error:', error);
          alert('❌ Error de conexión al intentar generar el PDF.');
        });
    }
...
// Luego continúa con
          ...
          data.filter(tr => {
            ...
            row.innerHTML = `
              ...
              <td>
                ...
                <button onclick='listar(${tr.id})' class='listar'>📋</button>
              </td>
            `;
    ...
    <footer style="background:#d4edda;text-align:center;padding:20px;" id="contacto" style="background:#1c1c1c;color:#eee;padding:20px;text-align:center;margin-top:40px;">
        <p><strong>Dirección:</strong> Calle 15 No. 8A-22 C.C. San Francisco Bogotá Colombia</p>
        <p><strong>Teléfono:</strong> +57 300 4451636</p>
        <p><strong>Email:</strong> martinezcorreayesid@gmail.com</p>
      ...
      });" style="color:#aaa;font-size:14px;">↑ Volver arriba</a>
          </div>
      </footer>
</body>
</html>
